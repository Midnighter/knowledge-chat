:param url=>"https://raw.githubusercontent.com/neo4j-graph-examples/star-wars/main/data/swapi.json"

create constraint film_url_unique for (f:Film) require f.url is unique;
create constraint character_url_unique for (c:Character) require c.url is unique;
create constraint planet_url_unique for (p:Planet) require p.url is unique;
create constraint starship_url_unique for (s:Starship) require s.url is unique;
create constraint vehicle_url_unique for (v:Vehicle) require v.url is unique;
create constraint species_url_unique for (s:Species) require s.url is unique;
